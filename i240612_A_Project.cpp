...
void drawBullet(RenderWindow& window, float& bullet_x, float& bullet_y, Sprite& bulletSprite);
void moveBees(float beeX[], float beeY[], float beeSpeedX[], float beeSpeedY[], float deltaTime, int numBees);
void drawBees(RenderWindow& window, Sprite& beeSprite, float beeX[], float beeY[], int numBees);
void movePlayer(float& player_x, float& player_y, float deltaTime);
void drawHoneycomb(RenderWindow& window, Sprite& honeycombSprite, float honeycomb_x, float honeycomb_y);
bool bulletAndBeeCollision(float bullet_x, float bullet_y, float beeX, float beeY, float boxPixelsX);
bool bulletAndHoneycombCollision(float bullet_x, float bullet_y, float honeycomb_x, float honeycomb_y, float boxPixelsX);
...
void display_menu(int& score) {
    ...
    //Text objects
    Text startGame("1. Start Game", font, 30);//first option
    startGame.setPosition(350, 200);
    startGame.setFillColor(sf::Color::White);

    Text viewScore("2. View Score", font, 30);//second option
    viewScore.setPosition(350, 300);
    viewScore.setFillColor(sf::Color::White);

    Text exitGame("3. Exit Game", font, 30);//third option
    exitGame.setPosition(350, 400);
    exitGame.setFillColor(sf::Color::White);
    ...
    while (window.isOpen()) {// game loop
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed) {
                if (event.key.code == sf::Keyboard::Num1) {// if the player presses 1, it starts the game
                    window.close();

                }
                else if (event.key.code == sf::Keyboard::Num2) {//player presses 2, it shows scores
                    window.close();
                    display_score(score); // Show scores
                }
                else if (event.key.code == sf::Keyboard::Num3) {//player presses 3, it exits the game
                    exit(0);
...
void level1()//function for level 1
{
  ...
  // float flower_y = player_y;
    ...
    while (window.isOpen()) {
        ...
        window.draw(background);
        drawPlayer(window, player_x, player_y, playerSprite);
        drawBees(window, beeSprite, beeX, beeY, numBees);
...
void level2()//function of level 2
{
    ...
    while (window.isOpen()) {
        ...
        drawPlayer(window, player_x, player_y, playerSprite);
        drawBees(window, beeSprite, beeX, beeY, numBees);
        drawBees(window, killerbeeSprite, killerBeeX, killerBeeY, killerBees);
        if (bullet_exists)
            drawBullet(window, bullet_x, bullet_y, bulletSprite);
        // Draw all active honeycombs
        for (int i = 0; i < honeycombCount; i++) {
...
//function to control movement of bees
void moveBees(float beeX[], float beeY[], float beeSpeedX[], float beeSpeedY[], float deltaTime, int numBees) {
    for (int i = 0; i < numBees; i++) {
        beeX[i] += beeSpeedX[i] * deltaTime;   //as distance = speed*time 
        if (beeX[i] < 0 || beeX[i] > resolutionX) { //edge cases
            beeY[i] += boxPixelsY;//moving one row down
            beeSpeedX[i] = -beeSpeedX[i];//changing direction 
            if (beeX[i] < 0) { beeX[i] = 0; }//if plaer tries to go out of window, he cannot go beyond the edges
            if (beeX[i] > resolutionX - boxPixelsX) { beeX[i] = resolutionX - boxPixelsX; }
        }
    }
}
